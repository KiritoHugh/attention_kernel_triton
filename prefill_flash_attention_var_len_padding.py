...
def flash_attention_kernel(
    ...
    cur_blklen = tl.load(var_blklen_ptr + batch_id)
    if seq_blk_id < cur_blklen:
...
def flash_attention_prefill(Q, K, V, causal, softmax_scale, GQA_group_size=1, varlen=None):
    ...
    flash_attention_kernel[grid](
        ...
        V,
        O,
        triton.cdiv(varlen, VAR_Q_SEQ_BLK_SIZE),
        softmax_scale,
...
def test_op_var_len_padding(BATCH_SIZE, NUM_HEADS_KV, SEQ_LEN, HEAD_DIM, causal, GQA_group_size = 1, dtype=torch.float16):
    ...
    def triton_implementation(Q, K, V, causal, softmax_scale, GQA_group_size, varlen):
        return flash_attention_prefill(Q, K, V, causal, softmax_scale, GQA_group_size, varlen)

    # compare
    ref_O = reference_implementation(Q, K, V, causal, softmax_scale, GQA_group_size)
    tri_O = triton_implementation(Q, K, V, causal, softmax_scale, GQA_group_size, Ls)
    # set zero by Ls
    ...
    tri_ms = triton.testing.do_bench(lambda: triton_implementation(Q, K, V, causal, softmax_scale, GQA_group_size, Ls))
